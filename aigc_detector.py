...
class AIGCDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        wav = load_wav(path, target_sr=self.sr)
        if self.train:
            # 训练：按比例随机裁剪 + 轻微增益抖动
            wav = self._ratio_crop(wav)
            wav = self._rand_gain(wav)
        else:
            # 验证/测试：若过长，居中裁到 eval_max_samples（默认 6s）
            if len(wav) > self.eval_max_samples:
                s = (len(wav) - self.eval_max_samples) // 2
                wav = wav[s: s + self.eval_max_samples]
        feat = wav_to_logmel(wav, sr=self.sr, n_mels=64)
...
def collate_fn(batch):
    ...
    feat_pad, lengths = pad_stack(feats, pad_value=0.0)  # [B, M, T], [B]
    labels = torch.stack([b["label"] for b in batch], dim=0)  # [B]
    utts = [b["utt"] for b in batch]
    paths = [b["path"] for b in batch]
    return {"utt": utts, "path": paths, "feat": feat_pad, "lengths": lengths, "label": labels}
...
class AIGCDetector(nn.Module):
    def __init__(self, n_mels: int = 64, cnn_width: int = 64, gru_hidden: int = 256,
                 num_heads: int = 4, drop: float = 0.2):
        super().__init__()
        # CNN 频谱前端（减频率维/提鲁棒性）
        self.cnn = nn.Sequential(
            ConvBlock(1, cnn_width, k=3, p=1, pool=(2,1)),     # [B, M/2, T]
            ConvBlock(cnn_width, cnn_width*2, k=3, p=1, pool=(2,1)),  # [B, M/4, T]
            ConvBlock(cnn_width*2, cnn_width*2, k=3, p=1, pool=(2,1)),# [B, M/8, T]
        )
        self.cnn_out_ch = cnn_width*2
        self.dropout = nn.Dropout(drop)

        # 将频率维聚合为通道（平均池化），得到 [B, T, C]
...
def train(
    ...
    epochs: int = 5, batch_size: int = 32, lr: float = 1e-6, weight_decay: float = 1e-4,
    ...
    model = AIGCDetector(n_mels=64, cnn_width=64, gru_hidden=256, num_heads=4, drop=0.2).to(device)
