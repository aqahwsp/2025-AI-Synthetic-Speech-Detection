...
# ---- Focal Loss (binary, with logits, AMP-safe) ----
class FocalWithLogitsLoss(nn.Module):
    ...
    def forward(self, logits: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        """
        logits: [B, 1] or [B]
        targets: [B, 1] or [B] in {0,1}
        """
        targets = targets.view_as(logits).float()
        # CE with logits (stable)
...
def prob_consistency_loss(logits_a: torch.Tensor, logits_b: torch.Tensor, reduction="mean"):
    """
    一致性：让整段与短切片的 clip 概率接近（MSE(prob)）。
    logits_a/logits_b: [B,1] 或 [B]
    """
    pa = torch.sigmoid(logits_a).view(-1, 1)
    pb = torch.sigmoid(logits_b).view(-1, 1)
    loss = F.mse_loss(pa, pb, reduction=reduction)
    return loss

def attention_symmetric_kl(attn_a: torch.Tensor, mask_a: torch.Tensor,
                           attn_b: torch.Tensor, mask_b: torch.Tensor,
                           out_len: int = 128, eps: float = 1e-8) -> torch.Tensor:
    """
    对注意力图做对称 KL：将 [B,H,T] 通过线性插值对齐到同一长度，再做 KL(p||q)+KL(q||p) 的均值。
    attn_* : [B,H,T], 每个头沿 T 已经 softmax；mask_*: [B,T] (True=有效)
    """
    B, H, Ta = attn_a.shape
    Tb = attn_b.shape[-1]
    # 插值到统一长度
    def interp_and_norm(A):
        # A: [B,H,T] -> [B*H,1,T] -> interpolate -> [B,H,L]
        x = A.reshape(B * H, 1, A.shape[-1])  # [B*H,1,T]
        x = F.interpolate(x, size=out_len, mode="linear", align_corners=False)
        x = x.reshape(B, H, out_len)
        # 重新归一化为分布
        x = x.clamp_min(eps)
        x = x / (x.sum(dim=-1, keepdim=True) + eps)
        return x
    pa = interp_and_norm(attn_a)
    pb = interp_and_norm(attn_b)
    # 对称 KL
    kl_ab = (pa * (torch.log(pa + eps) - torch.log(pb + eps))).sum(dim=-1)  # [B,H]
    kl_ba = (pb * (torch.log(pb + eps) - torch.log(pa + eps))).sum(dim=-1)
    loss = 0.5 * (kl_ab + kl_ba).mean()  # 标量
    return loss
...
def train(
    ...
    lambda_short: float = 0.5, # 短切片分支权重
    use_focal: bool = True,  #  启用 Focal 代替 clip BCE
    focal_gamma: float = 3.0,  #  Focal 超参
    focal_alpha: float = 0.25,  #  Focal 类别平衡
    lambda_consis: float = 0.2,  #   clip 概率一致性损失权重
    lambda_attn_kl: float = 0.05,  #  注意力 KL 一致性损失权重
):
    set_seed(2025)
    ...
    model_hparams = dict(n_mels=64, cnn_width=96, cnn_depth=4,
                         gru_hidden=384, gru_layers=3, num_heads=6, drop=0.3)
    model = AIGCDetector(**model_hparams).to(device)
    ...
    # 主损失：Focal 或 BCE
    if use_focal:
        clip_criterion = FocalWithLogitsLoss(gamma=focal_gamma, alpha=focal_alpha, reduction="mean")
    else:
    ...
    for ep in range(1, epochs + 1):
        ...
        for it, batch in enumerate(dl_train, 1):
            ...
            with torch.cuda.amp.autocast(enabled=(device.startswith("cuda"))):
                ...
                loss_clip = clip_criterion(clip_logit, labels)
                ...
                if "feat_short" in batch:
                    ...
                    loss_clip_s = clip_criterion(clip_logit_s, labels)  # ★ 同样用 Focal/BCE
                    ...
                    # 一致性：clip 概率 MSE
                    consis_clip = prob_consistency_loss(clip_logit, clip_logit_s, reduction="mean")
                    # 一致性：注意力对称 KL
                    consis_attn = attention_symmetric_kl(attn, attn_mask, attn_s, attn_mask_s, out_len=128)
                    loss = loss \
                           + lambda_short * (loss_clip_s + alpha_topk * loss_topk_s + beta_ent * loss_ent_s) \
                           + lambda_consis * consis_clip \
                           + lambda_attn_kl * consis_attn
...
def infer_to_csv(
    ...
    # 兼容：既支持新 ckpt（带 hparams），也支持老式纯 state_dict
    if isinstance(state, dict) and "state_dict" in state:
        hps = state.get("hparams", {})
        default_hps = dict(n_mels=64, cnn_width=96, cnn_depth=4,
                           gru_hidden=384, gru_layers=3, num_heads=6, drop=0.3)
        default_hps.update(hps)
        model = AIGCDetector(**default_hps).to(device)
        model.load_state_dict(state["state_dict"], strict=True)
    else:
        # 老权重（仅 state_dict），必须用你训练时的那组超参
        model = AIGCDetector(n_mels=64, cnn_width=96, cnn_depth=4,
                             gru_hidden=384, gru_layers=3, num_heads=6, drop=0.3).to(device)
        model.load_state_dict(state, strict=True)
    model.eval()
...
if __name__ == "__main__":
    ...
    p_train.add_argument("--use_focal", action="store_true", help="use focal loss for clip")
    p_train.add_argument("--focal_gamma", type=float, default=2.0)
    p_train.add_argument("--focal_alpha", type=float, default=0.25)
    p_train.add_argument("--lambda_short", type=float, default=0.5)
    p_train.add_argument("--lambda_consis", type=float, default=0.2)
    p_train.add_argument("--lambda_attn_kl", type=float, default=0.05)
    ...
    if args.cmd == "train":
        train(
            ...
            alpha_topk=args.alpha_topk, beta_ent=args.beta_ent,
        ...
        infer_to_csv(
            ...
            device=args.device, thr=args.thr
        )
    elif args.cmd == "eval":
